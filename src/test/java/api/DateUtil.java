...
public final class DateUtil extends DateUtils{
     ...
     * 获取时间
     *
     * @param time HH:mm:ss 格式
     * @return HH:mm:ss 格式
     */
    public static Time getTime(String time, String oriformat) {
        if (time == null || "".equals(time)) return null;
        try {
            return new Time(new SimpleDateFormat
                    (StringUtils.isEmpty(oriformat) ? DateUtil.STAND_TIME_FORMAT : oriformat, Locale.CHINESE)
                    .parse(time).getTime());
        } catch (ParseException e) {
            return null;
        }
    ...
    public static Time getTime(String time) {
        return getTime(time, DateUtil.STAND_TIME_FORMAT);
    }

    /**
     * 获取Time
     *
     * @param hour   时    0 to 23
     * @param minute 分    0 to 59
     * @param second 秒    0 to 59
     * @return
     */
    ...
    public static Integer utilDate2Integer(java.util.Date date) {
         ...
         return StringUtils.isNotEmpty(dateStr) ? Integer.valueOf(dateStr) : null;
    }
...
//	}
     ...
     * 返回本月月的最后一天
     * @param date 指定日期
     * @param format 返回的格式，默认返回 yyyy-MM-dd
     * @return
     */
     ...
     * @return
     */
    public static String getMonthLastDay(java.util.Date date,String format) {     
        Calendar calendar = Calendar.getInstance();    
     ...
     * @return
     */
    public static String getMonthStartDay(java.util.Date date,String format) {     
        Calendar calendar = Calendar.getInstance();    
        calendar.setTime(date);
        calendar.set(Calendar.DAY_OF_MONTH, calendar     
    ...
    public static String getMonthDay(java.util.Date date,int inc,String format) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
